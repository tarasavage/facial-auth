...
class TestTokenUtils:
    ...
    def test_access_token_decode(self, test_payload):
        ...
        invalid_token = create_access_token({"sub": "test2"})
        ...
        with pytest.raises(jwt.InvalidTokenError):
            decode_jwt(invalid_token_2)

    def test_decode_jwt_incorrect_arguments(self):
        for arg in [None, 1, True, False, 1.0]:
            with pytest.raises(ValueError) as e:
                decode_jwt(token="e.d.f", key="e.d.f", algorithm=arg)

            assert str(e.value) == "Algorithm must be a string"

        with pytest.raises(ValueError) as e:
            decode_jwt(token="e.d.f", key="e.d.f", algorithm=["HS256", "HS384"])

        assert str(e.value) == "Algorithm must be a string"

        with pytest.raises(ValueError) as e:
            decode_jwt(token="e.d.f", key="e.d.f", algorithm="None")

        assert str(e.value) == "Secure algorithm is required"

        with pytest.raises(ValueError) as e:
            decode_jwt(token=123, key="e.d.f", algorithm="HS256")
